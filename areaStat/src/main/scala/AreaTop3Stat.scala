...
object AreaTop3Stat {
  ...
  def getCityAreaInfo(sparkSession: SparkSession, taskParam: JSONObject)={
  ...
  def getAreaProductClickCountTable(sparkSession: SparkSession)= {
    ...
    "group_concat_distinct(concat_long_stringg(city_id , city_name ,':')) city_infos " +
  ...
  def main(args: Array[String]): Unit = {
    ...
    val cityId2AreaInfoRDD: RDD[(Long, CityAreaInfo)] = getCityAreaInfo(sparkSession,taskParam)

    getAreaPidBasicInfoTable(sparkSession,cityId2PidRDD,cityId2AreaInfoRDD)

    sparkSession.udf.register("concat_long_stringg",(v1:Long,v2:String,split:String)=> {
      v1 + split + v2
    })
